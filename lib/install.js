...
async function installCEPlugin() {
  ...
  try {
    ...
    // 安装 CE Plugin
    execSync('bun x @every-env/compound-plugin install compound-engineering --to openclaw', { 
      stdio: 'inherit' 
    });
...
async function createWorkspace(workspace, agent) {
  ...
  const dirs = [
    ...
    'life/projects/skill-dependency',
    'work/plans',
    'work/brainstorms',
    'work/todos',
    'docs',
  ...
  // 复制 life/projects/ 脚本
  copyProjectScripts(workspace);
  
  // 复制 para-system/ 脚本
  copyParaSystemScripts(workspace);
 ...
 * 复制 life/projects/ 脚本
 */
function copyProjectScripts(workspace) {
  const projectScripts = [
    { src: 'life/projects/pattern-extraction/weekly_pattern_extractor.py', dest: 'life/projects/pattern-extraction/weekly_pattern_extractor.py' },
    { src: 'life/projects/knowledge-validation/knowledge-cleanup.py', dest: 'life/projects/knowledge-validation/knowledge-cleanup.py' },
    { src: 'life/projects/nighttime-optimizer/nighttime_optimizer.py', dest: 'life/projects/nighttime-optimizer/nighttime_optimizer.py' },
    { src: 'life/projects/decision-logging/decision_logger.py', dest: 'life/projects/decision-logging/decision_logger.py' },
    { src: 'life/projects/motivation/motivation_tracker.py', dest: 'life/projects/motivation/motivation_tracker.py' },
    { src: 'life/projects/skill-dependency/skill_dependency_analyzer.py', dest: 'life/projects/skill-dependency/skill_dependency_analyzer.py' }
  ];
  
  for (const { src, dest } of projectScripts) {
 ...
 * 复制 para-system/ 脚本
 */
function copyParaSystemScripts(workspace) {
  const paraScripts = [
    { src: 'para-system/checkpoint-memory-llm.sh', dest: 'para-system/checkpoint-memory-llm.sh' },
    { src: 'para-system/nightly-deep-analysis.sh', dest: 'para-system/nightly-deep-analysis.sh' }
  ];
  
  for (const { src, dest } of paraScripts) {
    const srcPath = path.join(TEMPLATES_DIR, src);
    const destPath = path.join(workspace, dest);
